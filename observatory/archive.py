...
def create(base_path, model, version, experiment, run_id):
    ...
    model_folder = path.join(base_path, model, str(version),
                             experiment, run_id)

    if not path.exists(model_folder):
        raise AssertionError('Specified model data could not be found.')
    ...
    # we know this is valid since the folder exists :-)
    ...
    with tarfile.open(output_file, 'w:gz') as archive_file:
        for item in listdir(model_folder):
            archive_file.add(path.join(model_folder, item), item,
                             recursive=True)
